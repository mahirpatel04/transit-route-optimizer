...
// serviceIDs. One candidate per route — a later trip on the same route can
// never be a better edge than the earliest one, since NYC subway trips on
// the same route+direction visit stops in the same order.
func nextDeparturesByRoute(ctx context.Context, conn *pgx.Conn, stopID string, afterGTFSTime time.Duration, serviceIDs []string) ([]candidateTrip, error) {
	...
	rows, err := conn.Query(ctx, `
		SELECT DISTINCT ON (t.route_id) st.trip_id, t.route_id, st.stop_sequence, st.departure_time
		FROM stop_times st
	...
	for rows.Next() {
		var c candidateTrip
		if err := rows.Scan(&c.TripID, &c.RouteID, &c.StopSequence, &c.DepartureTime); err != nil {
